...
-- 3.TB_CATAGORY테이블의 NAME 컬럼에 PRIMARY KEY 를 생성하시오.
--(KEY 이름을 생성하지 않아도 무방함. 만일 KEY 이를 지정하고자 한다면 이름은 본이이
--알아서 적당한 이름을 사용한다.)

ALTER TABLE TB_CATEGORY 
    ADD CONSTRAINT 최인호 PRIMARY KEY(NAME);

DESC TB_CATEGORY;
    
-- 4. TB_CLASS_TYPE 테이블의 NAME 컬럼에 NULL값이 들어가지 않도록 속성을 변경하시오.
ALTER TABLE TB_CLASS_TYPE
    MODIFY NAME CONSTRAINT 최인호1 NOT NULL;
    
    
DESC TB_CLASS_TYPE;

-- 5. 두 테이블에서 컬럼 명이 NO 인 것은 기존 타입을 유지하면서 크기는 10 으로, 컬럼명이 NAME 인 것은 마찬가지로 기존타입을 유지하면서 크기 20 으로 변경하시오.
ALTER TABLE TB_CATEGORY
...
ALTER TABLE TB_CATEGORY 
    RENAME CONSTRAINT 최인호 TO PK_CATEGORY_NAME;
...
-- 9. TB_DEPARTMENT 의 CATEGORY 컬럼이 TB_CATEGORY 테이블의
-- CATEGORY_NAME 컬럼을 부모 값으로 참조하도록 FOREIGH KEY 를 지정하시오.
...
--11. 춘 기술대학교는 1년에 두번씩 과별로 학생과 지도교수가 지도 면담을 진행한다.
--이를 위해 사용할 학생이름, 학과이름, 담당교수이름 으로 구성되어 있는 VIEW를 만드시오.
--이때 지도 교수가 없는 학생이 있을 수 있음을 고려하시오 (단, 이 VIEW 는 단순SELECT 만을 할 경우 학과별로 정렬되어 화면에 보여지게 만드시오.)
...
-- 최근 3년을 기준으로 수강 인원이 가장 많았던 3 과목을 찾는 구문을 작성해보시오.
